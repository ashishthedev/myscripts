...
from whopaid.util_whopaid import SelectBillsAfterDate, SelectBillsBeforeDate, GetAllBillsInLastNDays, RemoveTrackingBills, RemoveGRBills
...
class PersistentMonthlySmsDetails(Persistent):
  ...
  def sendSmsForMonthHavingThisDate(self, date):
    ...
    if raw_input("Send (y/n) ?").lower() != "y":
      return
    d = dict()
    bills = RemoveGRBills(RemoveTrackingBills([b for b in GetAllBillsInLastNDays(90)]))
